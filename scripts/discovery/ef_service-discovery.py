...
##################################
# ChangeLog:
#  20160421   LS    initial creation
#  20160708   LS    add mysql discovery
#  20160906   LS    add haproxy discovery
##################################
...
def jmx_discovery():
    ...
    if len(sys.argv) > 2:
        ...
        for i in stdout.split('\n'):
            cmd_jmx = "%s -jar /var/lib/ef_zabbix/bin/jmx_discovery.jar %s 127.0.0.1:%s zabbix_check zabbix_check" %(java_bin, zbx_req_data,i)
...
def mysqld_discovery():
    ...
    if len(sys.argv) >= 3 and zbx_req_data == 'slave': 
        ...
        for port in stdout.split('\n'):
            mysql_conn = "mysql --defaults-extra-file=/etc/zabbix/conf/mysql_credentials -P %s" %(port)
            cmd = "echo 'show slave status \G' | %s | awk -F': ' '/Slave_(IO|SQL)_Running:/{print $2 }' | grep -c 'Yes'" %(mysql_conn)
            rcode, stdout, stderr = execute(cmd)
            if int(stdout) > 0:
